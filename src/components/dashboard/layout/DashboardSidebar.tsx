...
export function DashboardSidebar() {
  ...
  // To avoid hydration mismatch, default to w-60 before mount, or use the class dynamically.
  // Actually, we'll use isCollapsed state and trust it once mounted. 
  // NextJS might show a quick flash but it's purely client-side so it's acceptable.
  const widthClass = isCollapsed ? 'w-16' : 'w-56'

  return (
    <aside className={`${widthClass} bg-verde-oscuro flex flex-col h-full text-blanco transition-all duration-300 relative shrink-0 z-20`}>
      <div className={`px-6 pt-10 pb-6 border-b border-blanco/10 flex items-center ${isCollapsed ? 'justify-center px-0' : 'justify-between'}`}>
        {!isCollapsed ? (
          <Image src="/logo.png" alt="Centrevit" width={75} height={20} className="w-[75px] h-auto object-contain brightness-0 invert" priority />
        ) : (
          <div className="w-8 h-8 flex items-center justify-center font-serif text-blanco text-xl font-bold rounded bg-blanco/10">C</div>
        )}
      </div>

      <button 
        onClick={toggleSidebar}
        className="absolute -right-3 top-16 bg-crema text-verde-oscuro rounded-full p-1 border border-crema-oscuro shadow-md hover:bg-blanco transition-colors z-30"
        title="Toggle Sidebar"
      >
        <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-4 h-4 transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''}`}>
      ...
      <nav className="flex-1 py-6 flex flex-col gap-1 px-3 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => {
          // Exact match fix: ONLY active if exactly matches, with slash tolerance
          const isActive = pathname === item.href || pathname === `${item.href}/`
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-md font-sans text-sm transition-colors ${isActive ? 'bg-blanco/10 text-blanco border-l-2 border-dorado' : 'text-blanco/60 hover:text-blanco hover:bg-blanco/5 border-l-2 border-transparent'} ${isCollapsed ? 'justify-center border-l-0 px-0' : ''}`}
      ...
      <div className="p-4 border-t border-blanco/10">
        <form action="/auth/signout" method="post">
          <button 
            type="submit" 
            className={`flex items-center text-left py-2 text-sm text-blanco/60 hover:text-blanco hover:bg-blanco/5 rounded-md font-sans transition-colors w-full ${isCollapsed ? 'justify-center px-0' : 'px-3 gap-3'}`}
            title={isCollapsed ? "Cerrar sesión" : undefined}
          >
            <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" /></svg>
